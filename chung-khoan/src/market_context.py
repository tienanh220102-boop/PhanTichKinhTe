...
def detect_market(stock_code: Optional[str]) -> str:
    ...
    # Vietnam stocks (HOSE/HNX/UPCoM) are addressed with a ``.VN`` suffix
    # (e.g. FPT.VN, ACV.VN). Must be checked before the US letter regex, which
    # would otherwise match the base ticker.
    if code.endswith(".VN"):
        return "vn"
...
_MARKET_GUIDELINES = {
    ...
    "vn": {
        "zh": (
            "- 本次分析对象为 **越南股**（胡志明 HOSE / 河内 HNX / UPCoM 上市股票，代码带 `.VN` 后缀）。\n"
            "- 请按越南市场语境分析，关注越南盾（VND）汇率、越南央行（SBV）政策、外资持股比例与外资额度（foreign room）、"
            "以及各板块涨跌停制度（HOSE ±7%、HNX ±10%、UPCoM ±15%）；UPCoM 流动性与信息披露弱于两大主板，需额外提示风险。"
            "不要套用 A 股专属的北向资金、龙虎榜、融资融券等概念。"
        ),
        "en": (
            "- This analysis covers a **Vietnam stock** (listed on HOSE / HNX / UPCoM; code carries a `.VN` suffix).\n"
            "- Use Vietnam-market context: VND FX, State Bank of Vietnam (SBV) policy, foreign ownership limits / foreign room, "
            "and board-specific daily price limits (HOSE ±7%, HNX ±10%, UPCoM ±15%). UPCoM has weaker liquidity and disclosure "
            "than the two main boards, so flag that risk explicitly. Do not apply China A-share-specific concepts such as "
            "Northbound flows, Dragon Tiger lists, or margin-financing narratives."
        ),
    },
}
