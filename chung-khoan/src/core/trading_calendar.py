...
# Note: 'vn' intentionally omitted from MARKET_EXCHANGE. Without a validated
# exchange-calendars code for HOSE/HNX/UPCoM, phase inference returns UNKNOWN
# and trading-day checks fail open (treated as open), matching prior behavior
# for unrecognized codes while still giving correct Vietnam local time.
...
def get_market_for_stock(code: str) -> Optional[str]:
    ...
    # Vietnam stocks use a ``.VN`` suffix (FPT.VN, ACV.VN). Check first so the
    # base ticker is not misread as a US letter code.
    if code.endswith(".VN"):
        return "vn"
