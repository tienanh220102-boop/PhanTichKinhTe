...
VN_BLUEPRINT = MarketStrategyBlueprint(
    region="vn",
    title="越南市场三段式复盘策略（HOSE/HNX/UPCoM）",
    positioning="聚焦 VN-Index/VN30 趋势、外资资金动向与板块轮动，覆盖两大主板与 UPCoM，形成次日交易计划。",
    principles=[
        "先看 VN-Index 与 VN30 方向，再看量能结构，最后看板块（银行、地产、证券、消费）持续性。",
        "区分 HOSE 主板、HNX 与 UPCoM 的流动性差异；UPCoM 信息披露弱、波动大，结论需附加风险提示。",
    ...
    dimensions=[
        StrategyDimension(
            name="趋势结构",
            objective="判断越南市场处于上攻、震荡还是防守阶段。",
            checkpoints=["VN-Index/VN30 是否同向", "银行与地产权重股是否支撑指数", "关键支撑阻力是否被突破"],
        ),
        StrategyDimension(
            name="资金与外资",
            objective="识别外资买卖超与外资持股上限（foreign room）对权重股的影响。",
            checkpoints=["外资净买卖方向", "热门股外资额度是否接近上限", "VND 汇率与 SBV 政策扰动"],
        ),
        StrategyDimension(
            name="板块与主题",
            objective="提炼可延续主线与需要规避的拥挤方向。",
            checkpoints=["银行/地产/证券是否轮动", "UPCoM 大盘股（ACV、VGI 等）是否异动", "新闻催化是否支撑价格行为"],
        ),
    ],
    action_framework=[
        "进攻：VN-Index/VN30 共振上行 + 权重银行地产确认 + 外资净流入。",
        "均衡：指数或权重股分化，控制仓位并等待确认。",
        "防守：外资转为净卖出或权重转弱，优先控制回撤，UPCoM 仓位从严。",
    ],
)
