...
def _is_vn_market(code: str) -> bool:
    """判定是否为越南股票代码（HOSE/HNX/UPCoM）。

    越南代码用 ``.VN`` 后缀寻址（如 FPT.VN / ACV.VN），避免与 1-5 位美股字母代码冲突。
    """
    return (code or "").strip().upper().endswith(".VN")
...
class DataFetcherManager:
    ...
    def _init_default_fetchers(self) -> None:
        ...
        from .vci_fetcher import VCIFetcher
        config = get_config()
        ...
        vci = VCIFetcher()          # 越南市场数据源（HOSE/HNX/UPCoM，公开 API，无需配置）
    ...
    def get_daily_data(
        ...
        is_tw = (not is_vn) and (not is_us) and (not is_hk) and _is_tw_market(stock_code)
        market = ("vn" if is_vn else "us" if is_us else "hk" if is_hk
                  else "jp" if is_jp else "kr" if is_kr else "tw" if is_tw else "cn")
        if market != "cn":
